(() => {
  ...
  const locationOK = (lat, lng) =>
    finite(lat) && Math.abs(lat) <= 90 && finite(lng) && Math.abs(lng) <= 180;
  function frameStats(vehicles) {
    const count = vehicles.length;
    const sum = vehicles.reduce((total, vehicle) => total + vehicle.speed, 0);
    return {
      count,
      meanSpeed: count ? sum / count : null,
  ...
  function validate(data, mode) {
    if (mode === "heatmap") {
      if (
        data.type !== "sumo_geo_heatmap" ||
        !Array.isArray(data.points) ||
        !data.points.length
      )
        throw new Error("Invalid SUMO heat export");
      for (const point of data.points) {
        if (
          !Array.isArray(point) ||
          !locationOK(point[0], point[1]) ||
          !finite(point[2]) ||
          point[2] < 0
        )
          throw new Error("Invalid heat sample");
      }
      return { records: data.points.length, frames: null, meanSpeed: null };
    }
    if (
      data.type !== "sumo_geo_simulation" ||
      !Array.isArray(data.timesteps) ||
      !data.timesteps.length
    )
      throw new Error("Invalid SUMO trajectory export");
    ...
    for (const frame of data.timesteps) {
      if (
        ...
        throw new Error("Invalid frame time or vehicles");
      previous = frame.time;
      const ids = new Set();
      for (const vehicle of frame.vehicles) {
        if (
          vehicle.id === undefined ||
          ids.has(String(vehicle.id)) ||
          !locationOK(vehicle.lat, vehicle.lng) ||
          !finite(vehicle.speed) ||
          vehicle.speed < 0
        )
    ...
    return {
      records,
      frames: data.timesteps.length,
      meanSpeed: records ? speedSum / records : null,
    };
  }
